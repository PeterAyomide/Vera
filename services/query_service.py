...
def _scope_chunks_by_party(question: str, chunks: List[dict]) -> List[dict]:
    """Boost or demote chunks based on whether their document parties match the question.

    When a question names specific parties (e.g. "Harrington & Cole and Luminary
    Ventures"), chunks from documents that involve those parties are boosted, and
    chunks from unrelated documents (e.g. an NDA between different parties) are
    demoted. This prevents cross-contract contamination where the wrong document's
    clause gets applied to answer a question about a different agreement.

    Scoring:
      +0.15  for each question party found in the chunk's [Parties: ...] label
      -0.20  if chunk has explicit parties and NONE of them match any question party
      No change if chunk has no party metadata (we don't penalise unlabelled docs)
    """
    ...
    for chunk in chunks:
        ...
        if chunk_parties:
            ...
            if matched > 0:
                adjustment = min(0.15 * matched, 0.30)  # cap boost at +0.30
            else:
                adjustment = -0.20  # penalise — wrong document for these parties

        current = float(c.get("similarity", 0))
        c["similarity"] = max(0.0, min(1.0, current + adjustment))
