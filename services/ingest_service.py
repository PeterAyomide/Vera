...
def _extract_text_docx(docx_bytes: bytes) -> str:
    """Extract text from .docx preserving paragraph breaks and table content.

    python-docx's `doc.paragraphs` silently skips table cells, so we walk
    the document body in order, emitting paragraphs as prose and tables as
    pipe-delimited rows so downstream chunking keeps them intact.
    """
    ...
    for block in doc.element.body:
        ...
        if tag == qn("w:p"):
            ...
            # Table — emit each row as "| cell | cell | ..." so the
            # markdown-table splitter in _chunk_text keeps it atomic.
